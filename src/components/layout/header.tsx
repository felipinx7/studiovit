...
export function Header() {
  ...
  return (
    <header className="z-[9999] flex h-auto items-center justify-between rounded-[30px] bg-zinc-300/20 px-7 max-md:px-4 py-4">
      {/* HEADER DESKTOP  */}
        ...
        >
          {textoLinkHeader.map((link) => (
            <a
              href={link.id}
              key={link.section}
              className="text-neutral-0/60 text-[1.1rem] hover:text-neutral-0"
            >
              {link.nomeLink}
            </a>
          ))}
